...
Combobox.Selection = Base => class extends Base {
  ...
  _select(option, { forceAutocomplete = false } = {}) {
    if (!this.isMultiple()) this._resetOptions()

    if (option) {
      if (!this.isMultiple()) this._autocompleteWith(option, { force: forceAutocomplete })
  ...
  _commitSelection(option, { selected }) {
    ...
    if (this.isMultiple()) return this._commitMultipleSelection(option, { selected })
  ...
  _markSelected(option, { selected }) {
    if (this.hasSelectedClass) option.classList.toggle(this.selectedClass, selected)
  ...
  _selectNew() {
    if (this.isMultiple()) return this._selectNewForMultiple(query)
  ...
  _preselectOption() {
    if (this.isMultiple()) return this._preselectMultipleOption()
